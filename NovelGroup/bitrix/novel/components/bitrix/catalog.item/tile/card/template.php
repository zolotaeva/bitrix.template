...
if(SITE_ID == "en"){
  ...
  if ($flSections['UF_PRODUCT_TITLE']) {
    //$APPLICATION->SetPageProperty("h1", $flSections['UF_PRODUCT_TITLE']);
  }
...
?>
  ...
  <?if($item['PROPERTIES']['MAIN_TITLE']['~VALUE'] != ""){?>
    <a href="<?=$item['DETAIL_PAGE_URL']?>" class="product_item_title -fs-25"><?=$item['PROPERTIES']['MAIN_TITLE']['~VALUE']?> <br><b><?=$item['NAME']?></b></a>
  <?}elseif ($flSections['UF_PRODUCT_TITLE']) {?>
    <a href="<?=$item['DETAIL_PAGE_URL']?>" class="product_item_title -fs-25"><?=$flSections['UF_PRODUCT_TITLE']?> <br><b><?=$item['NAME']?></b></a>
  <?}else{?>
    <a href="<?=$item['DETAIL_PAGE_URL']?>" class="product_item_title -fs-25"><?=$item['NAME']?></a>
  <?}?>
    <div class="products_list_row">
      <div class="col-product_info">
      <?if ($item['DISPLAY_PROPERTIES']){?>
        <div class="product_info -fs-14 -got-400">
        <?foreach($item['DISPLAY_PROPERTIES'] as $prop){?>
          <div class="product_info_row">
            <div class="product_info_col"><?=$prop['NAME']?></div>
            <div class="product_info_col"><?=$prop['VALUE']?></div>
          </div>
          <?}?>
        </div>
        <?}?>
      </div>
      <div class="col-product_text">
    ...
    <? if ($itemHasDetailUrl): ?>
      <div class="product_item_btn">
        <a href="<?=$item['DETAIL_PAGE_URL']?>" class="btn btn-link">Подробнее</a>
      </div>
    <? endif; ?>
  </div>
		...
		<span class="product-item-image-slider-slide-container slide d-none" id="<?=$itemIds['PICT_SLIDER']?>"
		...
		<span class="product-item-image-original d-none" id="<?=$itemIds['PICT']?>"
		...
		if ($item['SECOND_PICT'])
		{
			...
			?>
			<span class="product-item-image-alternative d-none" id="<?=$itemIds['SECOND_PICT']?>"
		...
		?>
		<div class="product-item-image-slider-control-container d-none" id="<?=$itemIds['PICT_SLIDER']?>_indicator"
			<?=($showSlider ? '' : 'style="display: none;"')?>>
			<?
			if ($showSlider)
			{
		...
		{
			?>
			<div class="product-item-image-slider-progress-bar-container d-none">
				...
				case 'quantityLimit':
					if ($arParams['SHOW_MAX_QUANTITY'] !== 'N')
					{
						if ($haveOffers)
						{
							...
							{
								?>
								<div class="product-item-info-container product-item-hidden d-none" id="<?=$itemIds['QUANTITY_LIMIT']?>"
